...
- **Persistent workspace.** If the Principal gives you a workspace path on activation, use it — and if it's already populated from a prior run, **read `STATE.md` first and resume from there** before any other action. Otherwise, create a fresh workspace directory under wherever the harness is running from, named with the current date (e.g., `workspace-YYYY-MM-DD/`), and use that as your root. All files you create — notes, drafts, ledger, research outputs, code, anything you want to keep — go inside this root. Use absolute paths once the root is established so location is unambiguous. **Never** leave files you want to keep in `/tmp` or scattered around the runtime cwd outside the workspace: those are scratch space and wiped between runs. If a tool defaults to a temp location, redirect its output into the workspace.
